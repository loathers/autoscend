string auto_combatDefaultStage3(int round, monster enemy, string text)
{
	...
	if(canUse($skill[Emit Matter Duplicating Drones]) && my_familiar() == $familiar[Grey Goose])
	{
		...
		if(my_location() == $location[The Hole in the Sky] && item_drops(enemy) contains $item[star] && items_drops(enemy) contains $item[hole] && needStarKey() && (item_amount($item[star]) < 8 && item_amount($item[line]) < 7) && drones >= 8){
			forceDrop = true;
		}
