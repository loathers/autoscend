...
string auto_edCombatHandler(int round, monster enemy, string text)
{
	...
	if (!isJarslberg())
	{
		abort("Not in Avatar of Jarlsberg, this combat filter will result in massive suckage.");
	}
	...
	if (round == 0)
	{
		...
		}*/ //Jarlsberg doesn't have resurrection mechanics
	}
	else
	{
	...
	set_property("auto_jarlsbergCombatRoundCount", 1 + get_property("auto_jarlsbergCombatRoundCount").to_int()); 
