...
boolean aosol_buySkills()
{
	...
	if(my_class == $class[Pig Skinner])
	{
		...
		{
			page = visit_url("choice.php?pwd=&whichchoice=1495&option=1", true);
		}
		if(!have_skill($skill[Blasetd Glutes]) && my_level()>=2 && my_meat()>200) //max hp +50%
		{
			page = visit_url("choice.php?pwd=&whichchoice=1495&option=2", true);
		}
		if(!have_skill($skill[Stretch]) && my_level()>=2 && my_meat()>200) //Stretched (10 advs, +75% Initiative)
		{
			page = visit_url("choice.php?pwd=&whichchoice=1495&option=3", true);
		}
		if(!have_skill($skill[Ball Throw]) && my_level()>=3 && my_meat()>300) //Deal your Mus in Phys Dmg
		{
			page = visit_url("choice.php?pwd=&whichchoice=1495&option=4", true);
		}
		if(!have_skill($skill[Strong Back]) && my_level()>=3 && my_meat()>300) //Passive Mus +20
		{
			page = visit_url("choice.php?pwd=&whichchoice=1495&option=5", true);
		}
